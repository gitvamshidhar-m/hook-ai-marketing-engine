...
export function exportSheets(result: AnalyzeResult) {  const headers = ["hook", "channel", "score", "psychology", "variation", "forecast_emotion"];
  const rows = [
    headers,
    ...result.hooks.map((h) => [
      h.text,
...
function everyHooksOf(campaigns: Campaign[]) {
  return campaigns.flatMap((c) =>
    c.result.hooks.map((h) => ({
      campaign: c.title,
      topic: c.topic,
      channel: h.channel,
      text: h.text,
...
// Hook Pack: compile every saved campaign's hooks into one clipboard-ready bank
// and a branded, print-to-PDF deliverable. Great for freelancers sending a client
// a hands-off "here's your opening bank."
export function exportHookPack(campaigns: Campaign[]) {
  if (!campaigns.length) return;
  const items = everyHooksOf(campaigns).sort((a, b) => b.score - a.score);
  const lines = items.map((i) => `[${i.score}/100] [${i.channel}] ${i.text}`);
  const packText =
    `${new Date().toLocaleDateString()} · Hook AI Hook Pack\n\n` + lines.join("\n");

  navigator.clipboard
    .writeText(packText)
    .then(() => alert(`Copied ${items.length} hooks to your clipboard as a Hook Pack.`))
    .catch(() => {
      ...
      alert(`Copied ${items.length} hooks to your clipboard as a Hook Pack.`);
    });
}

// Branded print-to-PDF report bundling the best hook from every saved campaign.
export function printHookPack(campaigns: Campaign[]) {
  if (!campaigns.length) return;
  const win = window.open("", "_blank");
  if (!win) return;
  const items = everyHooksOf(campaigns).sort((a, b) => b.score - a.score);

  win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>Hook Pack — Hook AI</title><style>
    *{box-sizing:border-box}
    body{font-family:system-ui,sans-serif;max-width:820px;margin:0 auto;padding:2.5rem;color:#18181b;line-height:1.5}
    .brand{display:flex;align-items:center;gap:.5rem;font-size:.8rem;font-weight:700;letter-spacing:.02em;color:#6366f1}
    .mark{background:linear-gradient(135deg,#6366f1,#8b5cf6,#d946ef);color:#fff;border-radius:.5rem;width:1.6rem;height:1.6rem;display:inline-flex;align-items:center;justify-content:center;font-size:.9rem}
    h1{font-size:1.6rem;margin:.6rem 0 0}
    .meta{color:#71717a;font-size:.85rem;margin-top:.25rem}
    .pack{display:flex;align-items:center;gap:.75rem;padding:.6rem 0;border-bottom:1px solid #e4e4e7}
    .pack .score{font-weight:800;color:#4f46e5;min-width:3.2rem}
    .pack .ch{color:#71717a;font-size:.8rem;min-width:5rem}
    .pack .txt{font-weight:600}
    .footer{margin-top:2.5rem;font-size:.7rem;color:#a1a1aa;text-align:center;border-top:1px solid #e4e4e7;padding-top:1rem}
  </style></head><body>`);
  win.document.write(`<div class="brand"><span class="mark">⚡</span> HOOK AI · HOOK PACK</div>`);
  win.document.write(`<h1>Hook Pack · ${new Date().toLocaleDateString()}</h1>`);
  win.document.write(`<p class="meta">${items.length} hooks across ${campaigns.length} campaigns, sorted best-first</p>`);
  items.forEach((i) => {
    win.document.write(
      `<div class="pack"><span class="score">${i.score}</span><span class="ch">${i.channel}</span><span class="txt">${i.text}</span></div>`
    );
  });
  win.document.write(`<div class="footer">Generated by Hook AI · hook-ai-marketing-engine.vercel.app</div>`);
  win.document.write("</body></html>");
