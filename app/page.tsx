...
async function fetchSocialProof(): Promise<{
  hooks: number;
  campaigns: number;
  signups: number;
  leads: number;
}> {
  const empty = { hooks: 0, campaigns: 0, signups: 0, leads: 0 };
    ...
    <main className="flex-1">
      ...
      <section className="relative overflow-hidden border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-20 text-center sm:pt-24">
          ...
          {/* Social proof strip */}
          {(social.hooks > 0 || social.campaigns > 0) && (
            <div className="animate-fade-up mx-auto mt-8 grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-4 [animation-delay:360ms]">
              <div className="rounded-2xl border border-zinc-200 bg-white/70 px-4 py-3 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70">
                <p className="text-2xl font-bold">{social.hooks.toLocaleString()}+</p>
                <p className="text-xs text-zinc-500">hooks generated</p>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-white/70 px-4 py-3 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70">
                <p className="text-2xl font-bold">{social.campaigns.toLocaleString()}+</p>
                <p className="text-xs text-zinc-500">campaign runs</p>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-white/70 px-4 py-3 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70">
                <p className="text-2xl font-bold">{social.signups.toLocaleString()}+</p>
                <p className="text-xs text-zinc-500">accounts created</p>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-white/70 px-4 py-3 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70">
                <p className="text-2xl font-bold">{social.leads.toLocaleString()}+</p>
      ...
      <section className="border-t border-zinc-200 bg-zinc-50/80 dark:border-zinc-800 dark:bg-zinc-950/50">
        <div className="mx-auto max-w-6xl px-4 py-14">
          <div className="grid gap-4 md:grid-cols-3">
            {[
              {
                quote: "I pasted my competitor's ads in and got a blue-ocean angle in seconds. Changed our whole launch.",
                name: "Performance Marketer",
                role: "D2C brand · 2nd run",
              },
              {
                quote: "The CTR score is scary good. A/B tests keep landing on the top-scored hook.",
                name: "Growth Lead",
                role: "B2B SaaS",
              },
              {
                quote: "Three hooks, a campaign plan, and a budget projection — all before my coffee went cold.",
                name: "Freelance Copywriter",
                role: "Running ads for 3 clients",
              },
            ].map((t) => (
              <figure
                key={t.name}
                className="card-elevated rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900"
              >
                <blockquote className="text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">“{t.quote}”</blockquote>
                <figcaption className="mt-4 flex items-center gap-2">
                  <span className="bg-gradient-brand flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold text-white">
                    {t.name.charAt(0)}
                  </span>
                  <span>
